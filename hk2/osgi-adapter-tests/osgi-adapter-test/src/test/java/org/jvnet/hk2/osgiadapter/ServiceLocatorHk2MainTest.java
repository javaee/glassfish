...
public class ServiceLocatorHk2MainTest {
	...
	public void testHK2Main() throws Throwable {
        ...
        assertEquals("org.jvnet.hk2.osgiadapter.HK2Main",main.getClass().getCanonicalName());
        hk2Tracker.close();
        ...
        assertEquals("org.jvnet.hk2.osgiadapter.OSGiModulesRegistryImpl", mr.getClass().getCanonicalName());
        
        final ServiceLocator serviceLocator = main.createServiceLocator(mr, startupContext);
        

        ModulesRegistry mrFromServiceLocator = serviceLocator.getService(ModulesRegistry.class);
        ...
        List<?> startLevelServices = serviceLocator.getAllServices(BuilderHelper.createContractFilter("org.osgi.service.startlevel.StartLevel"));        
        
        assertEquals(1, startLevelServices.size());
        
        List<?> startups = serviceLocator.getAllServices(BuilderHelper.createContractFilter(ModuleStartup.class.getCanonicalName()));
    	assertEquals("Cannot find ModuleStartup", 1, startups.size());
    	
        final ModuleStartup moduleStartup = main.findStartupService(mr, serviceLocator, null, startupContext);
  
        assertNotNull("Expected a ModuleStartup that was provisioned as part of this test", moduleStartup);
        
        moduleStartup.start();
        
} catch (Exception ex) {
	if (ex.getCause() != null) throw ex.getCause();
	...
	private void checkServiceLocatorOSGiRegistration(
		...
		ServiceReference serviceLocatorRef = bundleContext.getServiceReference(ServiceLocator.class.getName());

        ServiceLocator serviceLocatorFromOSGi = (ServiceLocator) bundleContext.getService(serviceLocatorRef);
        
        assertNotNull("Expected ServiceLocator to be registed in OSGi", serviceLocatorFromOSGi);
        assertEquals("Expected same ServiceLocator in OSGi as the one passed in", serviceLocator, serviceLocatorFromOSGi);
	}
