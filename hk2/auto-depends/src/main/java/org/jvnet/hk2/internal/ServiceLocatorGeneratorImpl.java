...
public class ServiceLocatorGeneratorImpl implements ServiceLocatorGenerator {
  ...
  public ServiceLocator create(String name, Module module) {
    ...
    try {
      ...
      Services services = HK2.get().create(null, new ServicesModuleImpl(allProviders));
      
      for (ExtendedProvider<?> provider : allProviders) {
        ...
        ((ExtendedProviderImpl<?>) provider).setServicesProvider(allMe);
      }
