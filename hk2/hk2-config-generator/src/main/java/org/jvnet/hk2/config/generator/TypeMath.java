...
class TypeMath {
    ...
    TypeMirror isCollection(TypeMirror t) {
        TypeMirror collectionType = baseClassFinder.apply(t, env.getTypeElement(Collection.class.getName()));
     ...
     */
    static final SimpleTypeVisitor6<TypeMirror,TypeElement> baseClassFinder = new SimpleTypeVisitor6<TypeMirror, TypeElement>() {

        @Override
        public TypeMirror visitDeclared(DeclaredType type, TypeElement sup) {
            TypeMirror r = onDeclaredType(type,sup);
            if(r!=null)     return r;

            // otherwise recursively apply super class and base types
            if(((TypeElement) type.asElement()).getSuperclass() != null) {

                r = visitDeclared(((TypeElement) type.asElement()).getSuperclass(), sup);
                if(r!=null)     return r;
            }
        ...
        @Override
        public TypeMirror visitPrimitive(PrimitiveType type, TypeElement param) {
            return null;
        }

        @Override
        public TypeMirror visitNoType(NoType type, TypeElement param) {
            return null;
        }
        ...
        @Override
        public TypeMirror onTypeVariable(TypeVariable t, TypeElement sup) {
        ...
        @Override
        public TypeMirror visitArray(ArrayType type, TypeElement sup) {
        ...
        public TypeMirror visitWildcard(WildcardType type, TypeElement sup) {
            ...
            // so apply bounds recursively.
            for( ReferenceType r : type.getSuperBound() ) {
                TypeMirror m = visit(r,sup);
                if(m!=null)     return m;
            }
            return null;
     ...
     */
    final SimpleTypeVisitor6<JExpression,JExpression> SIMPLE_VALUE_CONVERTER = new SimpleTypeVisitor6<JExpression, JExpression>() {

        @Override
        public JExpression visitPrimitive(PrimitiveType p, JExpression param) {
        ...
        @Override
        protected JExpression defaultAction(TypeMirror type, JExpression param) {
            throw new UnsupportedOperationException("Unhandled type: " + type.toString());
        }
