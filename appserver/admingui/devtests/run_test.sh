...
test_run(){
  export PWD=$(openssl rand -base64 12)
  touch /tmp/password.txt
  chmod 600 /tmp/password.txt
  echo "AS_ADMIN_PASSWORD=" > /tmp/password.txt
  echo "AS_ADMIN_NEWPASSWORD=$PWD" >> /tmp/password.txt
  export AS_ADMIN_PASSWORDFILE=/tmp/password.txt
  $S1AS_HOME/bin/asadmin --user admin --passwordfile /tmp/password.txt change-admin-password
  $S1AS_HOME/bin/asadmin start-domain
  echo "AS_ADMIN_PASSWORD=$PWD" > /tmp/password.txt
