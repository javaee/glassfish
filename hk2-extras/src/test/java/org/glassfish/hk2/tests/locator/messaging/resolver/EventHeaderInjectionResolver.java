 ...
 */
@Service
public class EventHeaderInjectionResolver implements
        InjectionResolver<EventHeader> {
    ...
    public Object resolve(Injectee injectee, ServiceHandle<?> root) {
        if (!injectee.getRequiredType().equals(String.class)) return null;
        
        Set<Annotation> annotations = injectee.getRequiredQualifiers();
        for (Annotation annotation : annotations) {
            if (annotation.annotationType().equals(EventHeader.class)) {
                EventHeader eh = (EventHeader) annotation;
                return eh.value();
            }
        }
