 ...
 */
@Singleton
public class CustomContextImpl implements Context<CustomContext> {
    ...
    public <U> U findOrCreate(ActiveDescriptor<U> activeDescriptor,
            ServiceHandle<?> root) {
        if (instances.containsKey(activeDescriptor)) {
            return (U) instances.get(activeDescriptor);
        }
    ...
    public void destroyOne(ActiveDescriptor<?> descriptor) {
        if (instances.containsKey(descriptor)) {
            ((ActiveDescriptor<Object>) descriptor).dispose(instances.get(descriptor));
        }
    ...
    public void shutdown() {
        for (Map.Entry<ActiveDescriptor<?>, Object> entry : instances.entrySet()) {
            ActiveDescriptor<Object> d = (ActiveDescriptor<Object>) entry.getKey();
            Object destroyMe = entry.getValue();
