...
public class GlassfishNetworkListener extends GenericGrizzlyListener {
    ...
    protected void configureHttpProtocol(final Habitat habitat,
        ...
        AbstractActiveDescriptor<V3Mapper> onePortMapper = BuilderHelper.createConstantDescriptor(mapper);
        onePortMapper.addContractType(Mapper.class);
        onePortMapper.setName(address.toString() + port);
        
        ServiceLocatorUtilities.addOneDescriptor(grizzlyService.getHabitat(), onePortMapper);
