...
public class GuiUtil {
    ...
    public static void guiLog(String level, String message) {
        if (level == null) {
            level = "INFO";
        } else {
    ...
    public static void initSessionAttributes(){
        ...
        if (request instanceof javax.servlet.ServletRequest){
            ...
            String restServerName = (String) sessionMap.get(Constants.REST_SERVER_NAME);
            if (restServerName == null) {
                throw new IllegalStateException("REST Server Name not set!");
            }
            int port = (Integer) sessionMap.get(Constants.REST_SERVER_PORT);
        ...
        final String domainName = RestUtil.getPropValue((String) (sessionMap.get("REST_URL")), "administrative.domain.name", null, null);
        ...
        Map version = RestUtil.restRequest(sessionMap.get("REST_URL")+"/version", null, "GET" ,null, null, false);
        sessionMap.put("appServerVersion", ((Map)version.get("data")).get("message"));
        Map locations = RestUtil.restRequest(sessionMap.get("REST_URL")+"/location", null, "GET" ,null, null, false);
        ...
        Map runtimeInfoMap = RestUtil.restRequest(sessionMap.get("REST_URL")+"/get-runtime-info", null, "GET" ,null, null, false);
         ...
         */
        try {
            Map result = RestUtil.restRequest(GuiUtil.getSessionValue("REST_URL")+"/configs/config/server-config/admin-service/das-config", null, "GET", null, null, false);
 ...
 */
  ...
  //      return locale;
     ...
     */
    public static void prepareSuccessful() {
     ...
     */
    public static void prepareException(Throwable ex) {
...
*/
    ...
    public static void handleError(String detail) {
