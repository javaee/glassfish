...
/*
 * $Header: /m/jws/jmxcmd/src/com/sun/cli/jmxcmd/util/jmx/MBeanGenerator.java,v 1.8 2005/11/15 20:21:48 llc Exp $
 * $Revision: 1.8 $
...
public class MBeanGenerator implements MBeanGeneratorHook
{
	...
	{
		final Integer	count	= (Integer)mCounts.get( type );
		if ( count == null )
		{
			return( false );
		}
	...
	generate( final MBeanInfo	info, boolean emitComments )
	{
		...
		if ( attrInfos != null )
		{
			Arrays.sort( attrInfos, JMXUtil.MBeanAttributeInfoComparator.INSTANCE );
		...
		if ( operationInfos != null )
		{
			...
			{
				Arrays.sort( operationInfos, JMXUtil.MBeanOperationInfoComparator.INSTANCE );
