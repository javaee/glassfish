...
public class MBeanServerCmd extends JMXCmd
{
		...
		public
	MBeanServerCmd( final CmdEnv env )
	{
		super( env );
	}
	...
	{
		public	MBeanServerCmdHelp()	{ super( getCmdInfos() ); }
	...
	initMBeanServer(
		...
		Exception
	{
		for( int i = 0; i < initerClasses.length; ++i )
		{
			final MBeanServerIniter		initer	= (MBeanServerIniter)initerClasses[ i ].newInstance();
	...
	{
		final Class[]	mClasses;
		
		MBeansIniter( Class[]	classnames )
		{
			mClasses	= classnames;
		}
		
			public void
		initMBeanServer( String serverName , MBeanServer server ) throws Exception
		{
			for( int i = 0; i < mClasses.length; ++i )
			{
				final Object	mbean	= mClasses[ i ].newInstance();
				
				// create a unique object name (Object can change it in preRegister).
				ObjectName	objectName	= new ObjectName(
					":server=" + serverName +
					",classname=" + mClasses[ i ].getName() +
					",id=" + i );
		...
		private void
	doStart( String name, String[] initerClassnames, String[] mbeanClassnames )
		throws Exception
	{
		if ( findMBeanServer( name ) == null )
		{
			...
			initMBeanServer( name, server,
				new ClassSourceFromStrings( initerClassnames, false ).getClasses() );
			
			final ClassSource	mbeansClassSource	=
				new ClassSourceFromStrings( mbeanClassnames, false);
	...
	doList(  )
	{
		final java.util.Iterator iter	= getMBeanServerNames().iterator();
		...
		MBeanRegistrationException, NotCompliantMBeanException
	{
        ...
        final JMXConnectorServer connectorServer = (JMXConnectorServer)
	...
	private static int			sRMIRegistryPort	= 0;
		...
		private static void
	createRMIRegistry(final int port) throws RemoteException
	{
		...
		throws RemoteException
	{
		...
		private void
	loadHTMLAdapter( int adapterPort, String optionalName)
		throws MalformedURLException, IOException,
		...
		InvalidAttributeValueException
	{
		...
		try
		{
			final ObjectName	objectName	= optionalName != null ? new ObjectName( optionalName ) :
				new ObjectName( "adapter:name=html-adapter,protocol=html,type=adapter,port=" + adapterPort );
		...
		private boolean
	saslRequiresAclFile( String sasl )
	{
		...
		throws Exception
	{
		...
		final ObjectName	objectName	= firstOperand != null ? new ObjectName( firstOperand ) :
					new ObjectName( "connector:name=jmxmp-connector-server,type=connector,port=" + port );
		...
		InstanceNotFoundException, RemoteException
	{
        ...
        final RMIConnectorServer cs = (RMIConnectorServer)
            JMXConnectorServerFactory.newJMXConnectorServer(url, map, server);
           
		final ObjectName	objectName	= optionalName != null ? new ObjectName( optionalName ) :
			new ObjectName( "connector:name=rmi-connector-server,type=connector,rmi-registry-port=" + rmiRegistryPort );
