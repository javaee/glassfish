...
public class MonitorCmd extends JMXCmd
{
	...
	static final class MonitorCmdHelp extends CmdHelpImpl
	{
		...
		public	MonitorCmdHelp()	{ super( getCmdInfos() ); }
	...
	"";
		...
		public String	getName()		{	return( MONITOR_CMD ); }
		public String	getSynopsis()	{	return( formSynopsis( SYNOPSIS ) ); }
		public String	getText()		{	return( MONITOR_TEXT ); }
	}
		public CmdHelp
	getHelp()
	{
		...
		throws Exception
	{
		...
		if ( info.mFilename != null )
		{
			...
			output	= new CmdOutputN( new CmdOutput[] { fileOutput, stdout } );
		}
		else
		{
			output	= this;
	...
	getMonitor( final String name )
	{
		...
		void
	startMonitors( final String[] names )
		throws Exception
	{
		stopMonitors( names );
		...
		void
	deleteMonitors( final String[] names )
		throws Exception
	{
		stopMonitors( names );
		...
		throws Exception
	{
		...
		String
	getMonitorStatus( final String name )
	{
		String	status	= "";
		...
		void
	listMonitor( final MonitorInfo	m )
	{
		...
		throws Exception
	{
		...
		else if ( cmd.equals( DEFINE_CMD ) )
		{
			...
			final int	interval	= intervalSecondsOption == null ?
										DEFAULT_POLL_SECONDS : intervalSecondsOption.intValue();
	...
	class ChangeListenerImpl implements MonitorThread.ChangeListener
	{
		final CmdOutput	mOutput;
			...
			public
		ChangeListenerImpl( CmdOutput output )
		{
			mOutput	= output;
		}
	...
	final class MonitoringTarget
	{
				...
				InstanceNotFoundException, IntrospectionException
		{
				...
				) )
			{
				...
				final MBeanAttributeInfo[]	readOnly	= includeReadOnly ?
					JMXUtil.filterAttributeInfos( allInfos, ReadWriteAttributeFilter.WRITEABLE_FILTER ) : null;
				...
				final MBeanAttributeInfo[]	finalInfos	= (MBeanAttributeInfo[])
					candidateInfos.toArray( new MBeanAttributeInfo[ candidateInfos.size() ] );
...
class MonitorThread extends Thread implements CmdEventListener
{
	...
	long					getInterval()	{ return( mInfo.mIntervalSeconds ); }
	
	MonitoringTarget[]	getTargets()	{ return( mMonitoringTargets ); }
	...
	poll( final MonitoringTarget info )
