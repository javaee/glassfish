...
jmaki.charting.Common = {
   ...
   */
    createSeries : function(data, pData,  xWidth, _label, _color, _lbl) {
        ...
        var _st = new jmdojo.collections.Store();
        _st.setData(_j);
        var s = new jmdojo.charting.Series({
            dataSource:_st,
            bindings:{ x:"x", y:"y" },
            label: _lbl
        });
    ...
    createYAxis : function(wargs, high, low) {
        ...
        var yA = new jmdojo.charting.Axis();
    ...
    createXAxis : function(wargs, _count, _width, _pl, _pr, _datapoints) {
        ...
        var _lcount = 0;
        if (wargs.args && wargs.args.xAxis && wargs.args.xAxis.labels) _lcount = wargs.args.xAxis.labels.length;
        else if (wargs.value && wargs.value.xAxis && wargs.value.xAxis.labels) _lcount = wargs.value.xAxis.labels.length;
        // define the x-axis range either from input value or automatically
        var xA = new jmdojo.charting.Axis();
        xA.rotate = 0;
        if (xAxis.rotate){
        ...
        if (typeof xAxis != 'undefined') {
            ...
            if (typeof xAxis.labels[0] == 'string') {
                ...
                if (xAxis.labels[0] && !xAxis.labels[0].value) {
