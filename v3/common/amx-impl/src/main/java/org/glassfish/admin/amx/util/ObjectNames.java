...
public final class  ObjectNames
{
	...
	debug( final Object o )
	{
	    AMXDebug.getInstance().getOutput(
	        "org.glassfish.admin.amx.support.ObjectNames" ).println( o );
	}
    ...
    */
		...
		final String		childJ2EEType )
	{
		...
		return( buildContaineeObjectName( parentObjectName, parentFullType, childJ2EEType, name, false ) );
	}
	...
	*/
	...
	getFullType( final MBeanServer server, final ObjectName objectName )
	{
		try
		{
			final String fullType	= (String)server.getAttribute( objectName, AMXAttributes.ATTR_FULL_TYPE );
