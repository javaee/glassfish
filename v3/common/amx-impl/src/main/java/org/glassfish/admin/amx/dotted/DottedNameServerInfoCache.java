...
public class DottedNameServerInfoCache implements DottedNameServerInfo
{
	...
	DottedNameServerInfoCache( final DottedNameServerInfo src )
	{
		...
		mConfigNames	= new HashSet<String>();
		mServerNames	= new HashSet<String>();
		mServerToConfig	= new HashMap<String,String>();
		mConfigToServers	= new HashMap<String,String[]>();
	}
	...
	getConfigNameForServer( String serverName )
		throws DottedNameServerInfo.UnavailableException
	{
		return( mServerToConfig.get( serverName ) );
	}
	...
	getServerNamesForConfig( String configName )
		throws DottedNameServerInfo.UnavailableException
	{
		return( mConfigToServers.get( configName ) );
	}
	...
	{
		mConfigNames.clear();
        mConfigNames.addAll( mSrc.getConfigNames() );
        
		mServerNames.clear();
        mServerNames.addAll( mSrc.getServerNames() );
		
		// create mapping from server to config
        for( final String serverName : mServerNames )
		{
        ...
        for( final String configName : mConfigNames )
		{
