...
public class NotificationListenerBaseTest
    ...
    registerMBean(
        ...
        return getTestMBeanServer().registerMBean(mbean, objectName).getObjectName();
    }
    ...
    create(final String pattern)
        ...
        final MyListener listener =
                new MyListener(getTestMBeanServer(), JMXUtil.newObjectName(pattern), null);
    ...
    private final class MyRegListener
            extends MBeanRegistrationListener {
        public int mRegCount = 0;
        public int mUnregCount = 0;

        public MyRegListener(final ObjectName constrain)
                throws InstanceNotFoundException, IOException {
            super("MyRegListener", getTestMBeanServer(), constrain);
        }
    ...
    testListenForRegistration(
        ...
        for (final ObjectName objectName : s) {
            getTestMBeanServer().unregisterMBean(objectName);
        }
