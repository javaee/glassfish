...
public class Context extends StandardContext implements org.glassfish.web.embed.Context {
    ...
    public <T extends EventListener> void addListener(T t) {
        ...
        if (t instanceof ServletContextAttributeListener
            ...
            List<EventListener> oldEventListeners = getApplicationEventListeners();
            List<EventListener> eventListeners = new ArrayList<EventListener>();
            for (EventListener eventListener : oldEventListeners) {
                eventListeners.add(eventListener);
            }
            eventListeners.add(t);
            setApplicationEventListeners(eventListeners);
        } else if (t instanceof ServletContextListener
                || t instanceof HttpSessionListener) {               
            List<EventListener> oldLifecycleListeners = getApplicationLifecycleListeners();   
            List<EventListener> lifecycleListeners = new ArrayList<EventListener>();
            for (EventListener lifecycleListener : oldLifecycleListeners) {
                lifecycleListeners.add(lifecycleListener);
            }
            lifecycleListeners.add(t);
            setApplicationLifecycleListeners(lifecycleListeners);
        }  else {
    ...
    public void addListener(Class <? extends EventListener> c) {
        ...
        if (eventlistener instanceof ServletContextAttributeListener
            ...
            List<EventListener> oldEventListeners = getApplicationEventListeners();
            List<EventListener> eventListeners = new ArrayList<EventListener>();
            for (EventListener listener : oldEventListeners) {
                eventListeners.add(listener);
            }
            eventListeners.add(eventlistener);
            setApplicationEventListeners(eventListeners);
        } else if (eventlistener instanceof ServletContextListener
                || eventlistener instanceof HttpSessionListener) {               
            List<EventListener> oldLifecycleListeners = getApplicationLifecycleListeners();   
            List<EventListener> lifecycleListeners = new ArrayList<EventListener>();
            for (EventListener listener : oldLifecycleListeners) {
