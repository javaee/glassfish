...
then
	echo "Config directory is missing from this installation. Aboring uninstallation."
...
if [ ! -d ${CWD}/metadata ]
then
	echo "metadata directory is missing from this installation. Aboring uninstallation."
...
./uninstaller -s ${CWD}/var/install/config/glassfish -m file://${CWD}/metadata -p Default-Product-ID=glassfish -J "-Dorg.openinstaller.provider.configurator.class=org.openinstaller.provider.conf.InstallationConfigurator" $*
