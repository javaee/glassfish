 ...
 */
@Service
public class JvmConfigUpgrade implements ConfigurationUpgrade, PostConstruct {

    @Inject
    MonitoringService ms;

    private String level = ContainerMonitoring.LEVEL_OFF;
    private String name = ContainerMonitoring.JVM;

    public void postConstruct() {
        try {
            // get the level value for jvm attribute from module-monitoring-levels
            if (ms == null) {
                ...
                ConfigSupport.apply(new SingleConfigCode<MonitoringService>() {
                    ...
                    throws PropertyVetoException, TransactionFailure {
                        ContainerMonitoring newItem = param.createChild(
                            org.glassfish.admin.monitor.jvm.config.JvmMI.class);
