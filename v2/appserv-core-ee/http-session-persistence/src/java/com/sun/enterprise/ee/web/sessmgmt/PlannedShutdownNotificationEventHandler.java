...
public class PlannedShutdownNotificationEventHandler implements CallBack {
    ...
    public void processNotification(Signal notification) {        
        ...
        System.out.println("Received Planned Shutdown Notification: " + failedPartnerInstance);
        if(notification instanceof PlannedShutdownSignal){
            ...
            if(plannedShutdownSignal.getEventSubType() 
                == GMSConstants.shutdownType.GROUP_SHUTDOWN) {
                System.out.println("cluster shutting down:set stopping true");
                ReplicationHealthChecker.setStopping(true);
                ReplicationHealthChecker.setClusterStopping(true);
            }
            ...
            if(!ReplicationHealthChecker.isStopping()) {            
                ...
                System.out.println("planned shutdown notification causing call to respondToFailure");
                jxtaReplicationReceiver.respondToFailure(failedPartnerInstance, true);
            }            
        }         
    }     
