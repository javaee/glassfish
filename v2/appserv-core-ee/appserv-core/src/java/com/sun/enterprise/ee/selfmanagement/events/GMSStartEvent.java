 ...
 */
public class GMSStartEvent implements JoinNotificationActionFactory,
        JoinNotificationAction {
    
    /** Creates a new instance of GMSEvent */
    private  GMSStartEvent(GMSEventProxy proxy) {
        this.proxy = proxy;
    }
    
    static synchronized GMSStartEvent getInstance(GMSEventProxy proxy) {
        if (_instance != null)
           return _instance;
        else {
    ...
    public void consumeSignal(Signal s) throws ActionException { // JoinNotificationAction
        String memberToken = s.getMemberToken();
        if (proxy != null) {
            proxy.eventOccurred(memberToken,GMSEvent.START_EVENT);
        }
    ...
    private static volatile GMSStartEvent _instance = null;
