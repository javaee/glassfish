  ...
  (JNIEnv *env, jobject obj)
{
    ...
    //memStatus.dwLength = sizeof(memStatus); need to set for Ex version
    // XXX Use GlobalMemoryStatusEx for > 32 bit OS'es
    GlobalMemoryStatus(&memStatus);

    // Divide by 1024 = 2^10 for KB
    DWORD availMemKB = memStatus.dwAvailPhys >> 10;

    return (jlong)availMemKB;
}
