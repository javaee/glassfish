  ...
  (JNIEnv *env, jobject obj) 
{
    ...
    // sysinfo.loads[0] is 1 min CPU load avg, in the range 0 - 64k
    load = info.loads[0] / (double)65536 * (double)100;
    ...
    /* loadavg[0] is 1 min CPU load avg, in the range 0 - 1.0. */
    load = loadavg[0] * 100.0;

#endif
    //jdouble typedefed to double
    return (jdouble)load;
}
