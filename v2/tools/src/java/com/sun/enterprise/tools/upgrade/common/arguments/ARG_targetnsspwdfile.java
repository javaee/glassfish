 ...
 * @author Hans Hrasna
 */
public class ARG_targetnsspwdfile extends CertificateArgumentHandler {
    private String nssPwdFile;
    /** Creates a new instance of ARG_targetnsspwdfile */
    public ARG_targetnsspwdfile(ParsedArgument pa) {
        super(pa);
        if(parameters.isEmpty()){
            helpUsage(sm.getString("enterprise.tools.upgrade.cli.argument_missing" + " --targetnsspwdfile/-n"));
            System.exit(1);
        }
    ...
    protected void setCertificatePassword(String password) {
        ...
        commonInfo.setTargetNSSPwdFile(nssPwdFile);
        ...
        if(!validpwd) {
            commonInfo.recover();
            _logger.severe(sm.getString("enterprise.tools.upgrade.cli.Invalid_target_nss_password"));
            System.exit(1);
        }
        interactiveMap.put(ArgsParser.TARGETNSSPWD + "-" + commonInfo.getCurrentDomain(), password);
    }
}
