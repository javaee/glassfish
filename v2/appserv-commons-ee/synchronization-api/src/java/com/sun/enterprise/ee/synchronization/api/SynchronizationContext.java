...
/**
 * Synchronization context. It is intended for use by a remote server
 * instance with access to configuration (domain.xml). It provides 
 * access to the synchronization managers. The managers allow convenient
 * APIs to synchronize (pull) content from the central repository via
 * domain administration server (DAS). DAS connection information 
 * is obtained from das properties kept in each node agent.
 *
 * <xmp>
...
public interface SynchronizationContext {
    ...
    /** 
     * Sets the config context for this synchronization context.
     * 
    ...
    /**
     * Returns the config context associated with this synchronization context.
     * 
     * @return  config context associated with the synchronization context
     */
