...
/* WebContainerChangeHandler.java
 * $Id: WebContainerChangeHandler.java,v 1.3 2005/12/25 03:43:42 tcfujii Exp $
 * $Revision: 1.3 $
...
/**
 * Provides for dynamic reconfiguration of web container and related components like
 * servlets and web-module. This class decides the actions to take when there are
...
class WebContainerChangeHandler implements ChangeHandler {
	...
	private boolean isWebContainerType(MonitoredObjectType t) {
		return ( t == MonitoredObjectType.SERVLET		||
				 t == MonitoredObjectType.WEBMODULE);
	}
	
	private void handleChange(MonitoringLevel from, MonitoringLevel to,
                                  MonitoredObjectType t) {
	...
	private void notifyListeners(MonitoringLevel from, MonitoringLevel to, MonitoredObjectType t) {
		logger.finer("DynamicReconfigurator: Notifying the listeners for web container --- from = " + from.toString() + " to = " + to.toString());
