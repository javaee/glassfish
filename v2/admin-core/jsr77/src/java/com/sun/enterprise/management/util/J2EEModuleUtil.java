...
public class J2EEModuleUtil {
    ...
    public static String[] getjavaVMs()  {
        Set x = findNames("j2eeType=JVM");
        Iterator it = x.iterator();
        String vms[] = new String[x.size()];
        int i = 0;
        while(it.hasNext()) 
    ...
    public static Set findNames(String keys)  {
        Set s = new HashSet();
        ObjectName namepattern = null;
        try {
            String pattern = getDomainName() + ":" + keys + ",*";
            namepattern = new ObjectName(pattern);
        }
        catch(Exception e) {
            e.printStackTrace();
        }
