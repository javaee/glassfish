...
<xsl:template match="sch:schema | schema">
	...
	<xsl:template match="sch:name | name" mode="text">
		<axsl:text xml:space="preserve" xmlns:xml="http://www.w3.org/XML/1998/namespace"> </axsl:text>
	...
	<xsl:template match="sch:value-of | value-of" mode="text">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="process-value-of"><xsl:with-param name="select" select="'.'"/>
				</xsl:call-template>
			</xsl:otherwise>
                </xsl:choose>
		<axsl:text xml:space="preserve" xmlns:xml="http://www.w3.org/XML/1998/namespace"> </axsl:text>
...
<xsl:template name="diagnosticsSplit">
  ...
  <xsl:if test="string-length(normalize-space($start)) &gt; 0">
     <xsl:apply-templates select="//sch:diagnostic[@id = ($start) ] |
                          //diagnostic[@id= ($start) ]"/>
  </xsl:if>
...
<!-- ============================================================== -->
	...
	<xsl:template name="process-span"><xsl:apply-templates mode="inline-test"/></xsl:template>
