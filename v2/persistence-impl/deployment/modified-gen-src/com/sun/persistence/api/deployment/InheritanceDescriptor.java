/*
 * The contents of this file are subject to the terms 
 * of the Common Development and Distribution License 
 * (the "License").  You may not use this file except 
 * in compliance with the License.
 * 
 * You can obtain a copy of the license at 
 * glassfish/bootstrap/legal/CDDLv1.0.txt or 
 * https://glassfish.dev.java.net/public/CDDLv1.0.html. 
 * See the License for the specific language governing 
 * permissions and limitations under the License.
 * 
 * When distributing Covered Code, include this CDDL 
 * HEADER in each file and include the License file at 
 * glassfish/bootstrap/legal/CDDLv1.0.txt.  If applicable, 
 * add the following below this CDDL HEADER, with the 
 * fields enclosed by brackets "[]" replaced with your 
 * own identifying information: Portions Copyright [yyyy] 
 * [name of copyright owner]
 */

//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, vhudson-jaxb-1973 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// This file may need to modified upon recompilation of the source schema. 
// Generated on: 2005.04.20 at 08:27:00 IST 
//


package com.sun.persistence.api.deployment;

import javax.xml.bind.annotation.AccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;
import com.sun.persistence.api.deployment.DescriptorNode;
import com.sun.persistence.api.deployment.DiscriminatorType;
import com.sun.persistence.api.deployment.InheritanceType;

@XmlAccessorType(value = AccessType.FIELD)
@XmlType(name = "inheritance", namespace = "http://java.sun.com/xml/ns/persistence_ORM")
public class InheritanceDescriptor
    extends DescriptorNode
{

    @XmlElement(defaultValue = "SINGLE_TABLE", name = "strategy", namespace = "http://java.sun.com/xml/ns/persistence_ORM", type = InheritanceType.class)
    protected InheritanceType strategy;
    @XmlElement(defaultValue = "STRING", name = "discriminator-type", namespace = "http://java.sun.com/xml/ns/persistence_ORM", type = DiscriminatorType.class)
    protected DiscriminatorType discriminatorType;
    @XmlElement(defaultValue = "", name = "discriminator-value", namespace = "http://java.sun.com/xml/ns/persistence_ORM", type = String.class)
    protected String discriminatorValue;

    /**
     * Gets the value of the strategy property.
     * 
     * @return
     *     possible object is
     *     {@link com.sun.persistence.api.deployment.InheritanceType}
     */
    public InheritanceType getStrategy() {
        return strategy;
    }

    /**
     * Sets the value of the strategy property.
     * 
     * @param value
     *     allowed object is
     *     {@link com.sun.persistence.api.deployment.InheritanceType}
     */
    public void setStrategy(InheritanceType value) {
        this.strategy = value;
    }

    public boolean isSetStrategy() {
        return (this.strategy!= null);
    }

    public void unsetStrategy() {
        this.strategy = null;
    }

    /**
     * Gets the value of the discriminatorType property.
     * 
     * @return
     *     possible object is
     *     {@link com.sun.persistence.api.deployment.DiscriminatorType}
     */
    public DiscriminatorType getDiscriminatorType() {
        return discriminatorType;
    }

    /**
     * Sets the value of the discriminatorType property.
     * 
     * @param value
     *     allowed object is
     *     {@link com.sun.persistence.api.deployment.DiscriminatorType}
     */
    public void setDiscriminatorType(DiscriminatorType value) {
        this.discriminatorType = value;
    }

    public boolean isSetDiscriminatorType() {
        return (this.discriminatorType!= null);
    }

    public void unsetDiscriminatorType() {
        this.discriminatorType = null;
    }

    /**
     * Gets the value of the discriminatorValue property.
     * 
     * @return
     *     possible object is
     *     {@link java.lang.String}
     */
    public String getDiscriminatorValue() {
        return discriminatorValue;
    }

    /**
     * Sets the value of the discriminatorValue property.
     * 
     * @param value
     *     allowed object is
     *     {@link java.lang.String}
     */
    public void setDiscriminatorValue(String value) {
        this.discriminatorValue = value;
    }

    public boolean isSetDiscriminatorValue() {
        return (this.discriminatorValue!= null);
    }

    public void unsetDiscriminatorValue() {
        this.discriminatorValue = null;
    }

    //Added code

    InheritanceDescriptor() {
    }

    public void accept(Visitor v) throws DeploymentException {
        v.visitInheritanceDescriptor(this);
    }
    
}
