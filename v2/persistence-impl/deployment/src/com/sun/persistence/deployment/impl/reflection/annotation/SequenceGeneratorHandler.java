...
/**
 * This process SequenceGenerator annotation at package level
 *
 * @author Servesh Singh
 * @version 1.0
 */
public class SequenceGeneratorHandler extends HandlerBase {

    public SequenceGeneratorHandler(MergeManager mergeManager) {
        super(mergeManager);
    }

    public Class<? extends Annotation> getAnnotationType() {
        return javax.persistence.SequenceGenerator.class;
    }
    ...
    private void processAnnotation(Package pkg, DeploymentUnit du) {
        if (pkg.isAnnotationPresent(javax.persistence.SequenceGenerator.class)) {
            du.getPersistenceJar().setSequenceGenerator(
                    AnnotationToDescriptorConverter.convert(
                            pkg.getAnnotation(
