...
import com.sun.persistence.utility.DoubleLinkedList;
...
/**
 * <P>This class represents a connection manager, which creates a JDBC driver
 * manager and manages established database connections. This class lets you
 * specify the following settings for JDBC connections: <UL> <LI>JDBC driver
 * type <LI>data source name (JDBC URL) <LI>user name <LI>password <LI>number of
 * pooled connections (settable when running) <LI>how often to try to get a
 * connection after failing the first time and how long to retry (settable when
 * running) </UL> If you define a connection manager as a component, you can
 * define these settings when you partition the application instead of when you
 * write the application. <P>You can change only the following settings when the
 * connection manager is running: <UL> <LI> minimum and maximum number of pooled
 * connections, although not whether pooling is on or off
 * (<code>setMinPool</code> and <code>setMaxPool</code>) <LI> how often to try
 * to get a connection after failing the first time and how long to retry for a
 * connection (<code>setMsWait</code> and <code>setMsInterval</code>) </UL>
 ...
 * <code>shutDown</code> method, then change the settings. Then, start the
 * connection manager using the <code>startUp</code> method. <P>If you use a
 * connection manager to manage your database connections, the connection
 * manager can also extend a SynerJ transaction to include JDBC database
 * transactions. In other words, if the JDBC database transactions occur within
 * a SynerJ transaction, the JDBC database transactions are committed or rolled
 * back when the SynerJ transaction committed or rolled back. <P>You can set up
 * your connection manager to manage database connections in the following ways:
 * <UL> <LI>Start a new connection every time you request a connection. This
 * approach is often referred to as <i>client-based security</i>, because the
 * security for the connection is based on a particular database client and can
 * be different for each client. <LI>Maintain a pool of connections for a given
 * user name and password, and return one of these connections when you request
 * a connection using the getConnection method with no parameters. This approach
 * is often referred to as <i>application-based security</i>, because the
 * security information for the pool of connections is the same and is the same
 * for all clients of the application. <LI>Maintain a pool of connections for a
 * given user name and password and start a new connection if you specify
 * another user name and password with the getConnection method. This approach
 * is a blend of client-based and application-based security. </UL> <P>You also
 * have the choice of either defining the connection manager as a service object
 * typed as a ConnectionManager object, or defining the connection manager by
 * dynamically creating a ConnectionManager object in your code. If you define
 * the connection manager as a service object, the SynerJ partitioning system
 * can help you determine how to define partitions that contain the service
 * object by being aware of where JDBC drivers are installed, for example. If
 * you define the ConnectionManager object dynamically, then you need to keep
 * such issues in mind when you define the partitions whose code defines the
 * ConnectionManager objects; the partitioning system will not help you.
 *
 * <H4>Starting a New Database Connection for Each Request (Client-based
 * Security)</H4> <P>In this situation, the connection manager establishes a new
 * connection each time you request a connection. By default, the connection
 * manager does not establish and maintain a pool of connections. In this case,
 * you can leave the maximum and minimum number of pooled connections set to 0.
 * Each time you need a connection to the database, use the getConnection
 * method. You can use the default user name, password, and database URL for the
 * current connection manager by invoking the getConnection method with no
 * parameters. The default settings are specified in one of the
 * ConnectionManager constructors when you create the connection manager. You
 * can also specify a user name, password, and database URL on the getConnection
 * method.
 *
 * <H5>Example of Using ConnectionManager without Pooling</H4> <PRE> import
 * java.sql.*; import com.sun.jdo.api.persistence.support.*; Connection con =
 * myTransaction.getConnection(); Statement stmt = con.createStatement();
 * ResultSet rs = stmt.executeQuery("SELECT * FROM T1"); </PRE>
 *
 * <H4>Using a Pool of Database Connections (Application-based Security)</H4>
 * <P>When you create a connection manager using the ConnectionManager class,
 * you can have the connection manager establish a pool of connections for a
 * given user name and password to allow a thread to using an existing
 * connection instead of waiting for a new database connection to be created.
 * <P>To create a connection manager with a pool of connections using a service
 * object: <OL> <LI>Define a service object of the class ConnectionManager.
 * <LI>In the Partition Workshop, set the component properties for the component
 * instance represented by the service object to define the driver name and the
 * default database URL, user name, and password. You can also define the values
 * for the minimum connections and the maximum connections for the connection
 * pool, as well as for how often and how long to try to get a connection after
 * an initial failure to do so. <P>If you set the values for the minimum and
 * maximum connections to 0, then no pool of connections is established. Any
 * value greater than 0 means that a pool of connections is established. The
 * maximum value must be equal to or greater than the value of the minimum
 * value. If you set the maximum and minimum as equal values, the number of
 * connections established for the pool will be constant. <P>The connection
 * manager establishes the minimum number of connections when it starts up using
 * the default database URL, user name, and password. </OL> <P>To get one of the
 * pooled connections, you can use the <CODE>getConnection</CODE> method with no
 * parameters. If all the existing connections are in use, the connection
 * manager establishes another connection with the same database URL, user name,
 * and password and adds it to the pool, up to the specified maximum number of
 * connections. <P>When you have finished using a connection, you can use the
 * <CODE>close()</CODE> method to return the connection to the pool of available
 * connections. The connection manager periodically checks its pool of
 * connections, and can reduce the number of established connections to the
 * minimum number, if enough connections are not in use. At runtime, you can
 * change the maximum and minimum number of connections, because the
 * ConnectionManager is a component.
 *
 * <H4>Using a Pool of Connections and Starting New Connections</H4> <P>You can
 * have the connection manager establish and maintain a pool of connections and
 * have the connection manager establish new connections on a request-by-request
 * basis.
 *
 * <H5>Example of Using ConnectionManager with Pooling</H5> <PRE> import
 * com.sun.jdo.api.persistence.support.*; void getT1Data() { // The following
 * connection is from the connection pool. Connection myConn =
 * myTransaction.getConnection(); Statement myStatement =
 * myConn.createStatement(); ResultSet myResults = myStatement.executeQuery(
 * "SELECT * FROM T1); // Free the connection; it is returned to the connection
 * pool. myConn.close();
 *
 * // The connection manager creates a new connection for the // following
 * request. Connection yourConn = myConnMgr.getConnection(
 * "data:oracle:thin:@CUSTOMERDB:1521:ORCL", "paul", "omni8"); Statement
 * yourStatement = yourConn.createStatement(); ResultSet yourResults =
 * yourStatement.executeQuery( "SELECT Customer, Date, Amount FROM Orders"); . .
 * . } </PRE>
 */
public class ConnectionManager {
    ...
    /**
     * List of Connections associated with transactions, indexed by transaction
     * object (javax.transaction.Transaction).
     * @serial
     */
    ...
    /**
     * Default constructor. Creates a new connection manager and loads the
     * generic JDBC driver. You should typically use one of the other
     * constructors, because you cannot change JDBC drivers after the connection
    ...
    /**
     * Creates a new connection manager and loads the named JDBC driver. <p>
     * @param driverName name of JDBC driver.
     * @throws ClassNotFoundException if the driver cannot be found.
     * @throws SQLException if a SQL error is encountered.
     */
    public ConnectionManager(String driverName) throws ClassNotFoundException,
            SQLException {
        this();
        try {
     ...
     * the default database URL for the new connection manager. <p>
    ...
    /**
     * Creates a new connection manager, loads the named JDBC driver, and sets
     * the default database URL and user name for the new connection manager.
     * @param driverName the name of JDBC driver.
    ...
    /**
     * Creates a new connection manager, loads the named JDBC driver, and sets
     * the default database URL, user name, and password for the new connection
     * manager.
     * @param url the default database URL for the data source.
     * @param userName the default user name for database connections.
     * @throws ClassNotFoundException if the driver cannot be found.
     ...
     * connection pool sizes for the new connection manager. <P>If minPool and
     * maxPool are 0, connection pooling is disabled. If minPool is greater than
     * 0 and maxPool is greater than or equal to minPool, this constructor
     * creates a connection pool containing minPool connections.
     * @param url the default database URL for the data source.
     * @param userName the default user name for database connections.
     * @throws ClassNotFoundException if the driver cannot be found.
     * @throws SQLException if a SQL error is encountered or if the specified
     * value of minPool is not less than or equal to the specified value of
     * maxPool.
     * @param	driverName	the name of JDBC driver.
     * @param	password	the default password for database connections.
     ...
     */
    public ConnectionManager(String driverName, String url, String userName,
            String password, int minPool, int maxPool)
            throws ClassNotFoundException, SQLException {
        this();
        try {
            this.setDriverName(driverName);
     ...
     * defines the default values for the database URL, user name, password,
     * minimum and maximum connection pool sizes, and the length of time to wait
     * for a database connection. <P>If minPool and maxPool are 0, connection
     * pooling is disabled. If minPool is greater than 0 and maxPool is greater
     * than or equal to minPool, this constructor creates a connection pool
     * containing minPool connections. <P>If msWait is set to 0, the connection
     * manager does not try again to create or return a database connection if
     * the first try fails. For any other value, the connection manager waits
     * 1000 milliseconds (ms) (1 second) before trying again. If the msWait
     * value is less than 1000 ms, the connection manager waits 1000 ms before
     * trying. The connection manager continues trying until the value specified
     * by msWait is met or exceeded. <P>If you want to set the interval length
     * yourself, you can use the <code>ConnectionManager</code> constructor that
     * specifies the msInterval parameter or the <code>setInterval</code>
     * method.
     * @param url the default database URL for the data source.
     * @param userName the default user name for database connections.
     * @param msWait the total number of milliseconds to wait for a successful
     * connection.
     * @throws ClassNotFoundException if the driver cannot be found.
     * @throws SQLException if a SQL error is encountered or if the specified
     * value of minPool is not less than or equal to the specified value of
     ...
     */
    public ConnectionManager(String driverName, String url, String userName,
            String password, int minPool, int maxPool, int msWait)
            throws ClassNotFoundException, SQLException {
        this();
        try {
            this.setDriverName(driverName);
     ...
     * Creates a new connection manager, loads the named JDBC driver, and
     * defines the default values for the database URL, user name, password,
     * minimum and maximum connection pool sizes, the length of time to wait for
     * a database connection, and how frequently to try again to get a database
     * connection. <P>If minPool and maxPool are 0, connection pooling is
     * disabled. If minPool is greater than 0 and maxPool is greater than or
     * equal to minPool, this constructor creates a connection pool containing
     * minPool connections. <P>If msWait or msInterval is set to 0, the
     * connection manager does not try again to create or return a database
     * connection if the first try fails. <P>For any other values greater than
     * 0, the The connection manager continues trying after every specified
     * value for msInterval until the value specified by msWait is met or
     * exceeded. If the value for msInterval is greater than the value for
     * msWait, the connection manager tries again to return a connection once,
     * then fails if it could get a connection.
     * @param url the default database URL for the data source.
     * @param userName the default user name for database connections.
     * @param msWait the total number of milliseconds to wait to get a
     * connection.
     * @param msInterval the number of milliseconds to wait before trying again
     * to get a connection.
     * @throws ClassNotFoundException if the driver cannot be found.
     * @throws SQLException if a SQL error is encountered or if the specified
     * value of minPool is not less than or equal to the specified value of
    ...
    public ConnectionManager(String driverName, String url, String userName,
            String password, int minPool, int maxPool, int msWait,
            int msInterval) throws ClassNotFoundException, SQLException {
        this();
        try {
            this.setDriverName(driverName);
            this.setURL(url);
    ...
    /**
     * Establishes a connection to the default database URL using the default
     * user name and password. <P>If the current connection manager maintains a
     * connection pool, this method returns a pooled connection instead of
     * establishing a new connection. If all pooled connections are in use, and
     * the total wait time (msWait) for the connection manager and the retry
     * interval (msInterval) are not 0, then the connection manager tries to get
     * a database connection after the retry interval. The connection manager
     * continues to try until a pooled connection becomes available or until the
     * total time equals or exceeds the wait time. If the wait time expires
     * before the connection manager returns a database connection, this method
     * throws a <code>SQLException</code> exception with SQLState = "08006".
     *
     * <P>If the current connection manager is not set to try again for
     * connections (the wait time is 0) and no pooled connections are available,
     * this method throws a <code>SQLException</code> exception with SQLState =
     * "08006".
     *
     * <P>If the current connection manager is being shut down, this method
     * throws a <code>SQLException</code> exception with SQLState = "08003".
    ...
    public synchronized Connection getConnection() throws SQLException {
        ...
        {
            conn = (ConnectionImpl) this.getConnection(
    ...
    /**
     * Establishes a connection to the database at the default database URL
    ...
    public synchronized Connection getConnection(String userName,
        ...
        if (conn == null) {
            if (freeConn != null) {
                ...
                if (debug) {
                    logger.finest(
                            "connection.connectionmanager.found",
                            freeConn); // NOI18N
                }
                conn = freeConn;
                freeConn = null;
                ...
                try {
                    ...
                    conn = new ConnectionImpl(
                            DriverManager.getConnection(
                                    ...
                                    this.expandAttribute(password)), this.expandedUrl,
                            this.expandAttribute(userName), this);
                    if (debug) {
                        logger.finest(
                                "connection.connectionmanager.getnewconn",
                                conn); // NOI18N
                    }
                } catch (SQLException se) {
                    throw se;
    ...
    /**
     * Establishes a connection to the specified database URL using the
     ...
     * @throws SQLException if the connection fails.
     */
    public synchronized Connection getConnection(String url, String userName,
            String password) throws SQLException {
        boolean debug = logger.isLoggable(Logger.FINEST);
    ...
    /**
     * Check whether a ConnectionImpl is already associated with the transaction
     * on the current thread. <p>
     * @return ConnectionImpl associated with the transaction on the current
    ...
    /**
     * Starts up this ConnectionManager by loading the proper JDBC driver class
     * and initializing the pool if necessary. <p> You need to call this method
     * if you are using the ConnectionManager as a component, or if you use the
     * default constructor and set the attributes via the
     * <code>set</code><I>XXX</I> methods.
     * @throws ClassNotFoundException if the driver cannot be found.
     * @throws SQLException if a SQL error is encountered.
     */
    ...
    /**
     * Disconnects all free database connections managed by the current
     * connection manager and sets the shutDownPending flag to true. All busy
     * connections that are not participating in a transaction will be closed
     * when a yieldConnection() is performed.  If a connection is participating
     * in a transaction, the connection will be closed after the transaction is
    ...
    public synchronized void shutDown() throws SQLException {
        ...
        if (this.pooling == true) {
            ...
            this.pooling = false;
            this.initialized = false;
            for (conn = (ConnectionImpl) this.freeList.getHead();
                 conn != null; conn = (ConnectionImpl) conn.getNext()) {
                try {
                    conn.close();
     ...
     * @throws SQLException if the driverName is NULL.
     * @see #getDriverName
     */
    ...
    /**
     * Gets the default database URL for the data source. This default is only
     * for the current connection manager and was set by the
     * <code>ConnectionManager</code> constructor. This default is used if you
     * don't specify another database URL with a <code>getConnection</code>
     * method. To change this default value, use the <code>setURL</code>
     * method.
     ...
     * for the current connection manager. To get a connection using a different
     * data source than the default, use the <code>getConnection</code> method
     * that specifies a database URL as a parameter.
     ...
     * This default was set by the <code>ConnectionManager</code> constructor,
     * and is used if you don't specify another user name with a
     * <code>getConnection</code> method. To change this default value, use the
     ...
     * @param userName the default user name for the current connection
     * manager.
     * @see #getUserName
     */
     ...
     * This default was set by the <code>ConnectionManager</code> constructor,
     * and is used if you don't specify another password with a
     * <code>getConnection</code> method. To change this default value, use the
    ...
    /**
     * Gets the minimum number of pooled connections for the current connection
     * manager. If this value is 0, the connection manager does not maintain a
     * connection pool until a connection is requested using the getConnection
     * method with no parameters. <P>To change the minimum number of pooled
     * connections, use the <CODE>setMinPool</CODE> method.
    ...
    /**
     * Sets the minimum number of pooled connections for the current connection
     * manager. The default minimum number of pooled connections is 0, which
     * means that no connections are pooled until a pooled connection is
     * requested. <P>The specified value of the minPool parameter must be: <UL>
     * <LI>greater than or equal to 0. <LI>greater than or equal to the current
     * minimum number of pooled connections <LI>less than or equal to the
     * maximum number of pooled connections </UL> Otherwise, this method throws
     * a SQLException.
     * @param minPool the minimum number of pooled connections.
     * @throws SQLException if the connection manager is being shut down or if
     * the minPool value is not valid.
     * @see #getMaxPool
     * @see #getMinPool
     * @see #setMaxPool
     */
    ...
    /**
     * Gets the maximum number of pooled connections for the current connection
     * manager. If this value is 0, the connection manager does not maintain a
     * connection pool. When you request a connection with the
     * <CODE>getConnection</CODE> method, the <CODE>getConnection</CODE> method
     * always returns a new connection. <P>To change the maximum number of
     * pooled connections, use the <CODE>setMaxPool</CODE> method.
    ...
    /**
     * Sets the maximum number of pooled connections for the current connection
     * manager. The default maximum number of pooled connections is 0, which
     * means that no connections are pooled. <P>The specified value of the
     * maxPool parameter must be: <UL> <LI>greater than or equal to 0.
     * <LI>greater than or equal to the current maximum number of pooled
     * connections <LI>greater than or equal to the minimum number of pooled
     * connections </UL> Otherwise, this method throws a SQLException.
     * @param maxPool the maximum number of pooled connections.
     * @throws SQLException if the connection manager is being shut down or if
     * the maxPool value is not valid.
     * @see #getMaxPool
     * @see #getMinPool
     * @see #setMinPool
     */
    ...
    /**
     * Gets the amount of time, in milliseconds, the connection manager should
     * spend trying to get a pooled connection, which is the amount of time a
     * requester might wait. <p> This value is only meaningful when you use the
     * <code>getConnection</code> to get a pooled connection, which means that
     * no database URL, user name, or password is specified.
    ...
    /**
     * Sets the amount of time, in milliseconds, the connection manager should
     * spend trying to get a pooled connection, which is the amount of time a
     * requester might wait. Setting this value to 0 means that the connection
     * manager does not try again to get a database connection if it fails on
     * the first try. <p> This value is only meaningful when you use the
     * <code>getConnection</code> to get a pooled connection, which means that
     * no database URL, user name, or password is specified. <p> The connection
     * manager retries after the set interval until the total wait time is equal
     * to or greater than the specified wait time. You can determine the total
     * number of tries for a connection based on wait time and interval settings
     * using the following formula, where msWait is the wait time, msInterval is
     * the time between attempts to get a connection:
     * <pre>
     * tries = msWait/msInterval + 2
     * </pre>
     * For example, if msWait is set to 2000 ms and msInterval is set to 1500
     * ms, then the connection manager will try to get a database connection 3
     * times before throwing an exception if it has failed. <p> If the wait time
     * value is less than the set value for the interval between retries, but
     * not zero, the connection manager waits the amount of time specified by
     * the interval, tries once, then returns an exception if it still could not
     * get a connection.
    ...
    /**
     * Gets the amount of time, in milliseconds, between the connection
     * manager's attempts to get a pooled connection. <p> This value is only
     * meaningful when you use the <code>getConnection</code> to get a pooled
     * connection, which means that no database URL, user name, or password is
     * specified.
     ...
     * meaningful when you use the <code>getConnection</code> to get a pooled
     * connection, which means that no database URL, user name, or password is
     * specified. <p> The connection manager retries after the specified
     * interval until the total wait time is equal to or greater than the set
     * wait time. You can determine the total number of tries for a connection
     * based on wait time and interval settings using the following formula,
     * where msWait is the wait time, msInterval is the time between attempts to
     * get a connection:
     * <pre>
     * tries = msWait/msInterval + 2
     * </pre>
     ...
     * ms, then the connection manager will try to get a database connection 3
     * times before throwing an exception if it has failed. <p> If the wait time
     * value is greater than 0 but less than the set value for the interval
     * between retries, the connection manager waits the amount of time
     * specified by the interval, tries once, then returns an exception if it
     * still could not get a connection.
    ...
    /**
     * Associate a Connection with a transaction. <p>
    ...
    /**
     * Disassociate a Connection with a transaction. <p>
     * @param tran The Transaction's object.
     * @param conn The ForteJDBCConnect hosting the transaction.
     * @param free The ConnectionImpl should be returned to freePool.
     * @ForteInternal
     */
    synchronized void disassociateXact(Transaction tran, ConnectionImpl conn,
            boolean free) throws SQLException {
        ConnectionImpl xactConn = null;

        if (tran != null) {
            xactConn = (ConnectionImpl) this.xactConnections.remove(
                    (Object) tran);
        }
    ...
    /**
     * Expand connection pool by connections size. <p>
        ...
        for (int i = 0; i < connections; i++) {
            ...
            } else // There is room in the pool, so get a new connection.
            {
                try {
                    conn = new ConnectionImpl(
                            DriverManager.getConnection(
                                    this.expandedUrl, this.expandedUserName,
                                    this.expandedPassword), this.expandedUrl, this.expandedUserName, this);
                    conn.setPooled(true);
                    this.freeList.insertAtTail((Linkable) conn);
                    this.poolSize++;
                } catch (SQLException e) {
     ...
     * corresponding values; e.g, if url = ${MYURL}, expand ${MYURL} into its
    ...
    /**
     * Wait for a pool connection.  The thread will wait msInterval milliseconds
     * between tries to get a connection from the pool. If no connection is
     * available after msWait milliseconds, an exception is thrown. <p>
     * @throws SQLException if connection fails.
     * @ForteInternal
     */
     ...
     * Previous free connection can be released (closed) when a new one becomes
     * available.
     */
    protected synchronized void replaceFreeConnection(ConnectionImpl c) {
        ...
        if (debug) {
            logger.finest(
                    "connection.connectionmanager.replacefreeconn",
                    freeConn); // NOI18N
        }
        if (freeConn != null) {
