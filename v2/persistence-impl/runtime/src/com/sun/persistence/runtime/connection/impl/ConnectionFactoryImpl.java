...
import com.sun.persistence.support.JDOCanRetryException;
...
import com.sun.persistence.support.JDOUnsupportedOptionException;
...
public class ConnectionFactoryImpl implements ConnectionFactory,
     ...
     */
    public ConnectionFactoryImpl(String URL, String userName, String password,
            String driverName) {
    ...
    /**
     * Creates new <code>ConnectionFactoryImpl</code> object with user  and
     * connection info
     * @param URL connection URL
     * @param userName database user
     * @param password database user password
     ...
     */
    public ConnectionFactoryImpl(String URL, String userName, String password,
            String driverName, int minPool, int maxPool) {
        this.driverName = driverName;
        this.URL = URL;
        this.userName = userName;
        this.password = password;
            ...
            if (dbMetaData.supportsTransactionIsolationLevel(level)) {
                ...
                throw new JDOFatalException(
                        I18NHelper.getMessage(
                                messages, "connection.connectionfactoryimpl.isolationlevel_notsupported", //NOI18N
                                level));
            }
        } catch (Exception e) {
            throw new JDOFatalException(null, e);
        ...
        try {
            ...
            if (sqlState == null) {
                throw new JDODataStoreException(
                        I18NHelper.getMessage(
                                messages,
                                "connection.connectionfactoryimpl.sqlexception", "null",
                                "" + errorCode),
                        e); //NOI18N
            } else {
                throw new JDODataStoreException(
                        I18NHelper.getMessage(
                                messages,
                                "connection.connectionfactoryimpl.sqlexception",
                                sqlState, "" + errorCode),
                        e); //NOI18N
            }
        } catch (Exception e) {
            throw new JDOCanRetryException(
                    I18NHelper.getMessage(
                            messages, "connection.connectionfactoryimpl.getconnection"), e); //NOI18N
        }
    }

    /**
     * Determines whether obj is a ConnectionFactoryImpl with the same
     * configuration
     * @param obj The possibly null object to check.
     * @return true if obj is equal to this ConnectionFactoryImpl; false
     *         otherwise.
     */
    public boolean equals(Object obj) {
        if ((obj != null) && (obj instanceof ConnectionFactoryImpl)) {
            ConnectionFactoryImpl cf = (ConnectionFactoryImpl) obj;
            return (cf.URL.equals(this.URL)
                    && cf.userName.equals(this.userName)
                    && cf.driverName.equals(this.driverName)
                    && cf.password.equals(this.password));
        }
        return false;
    }
    ...
    private void assertNotConfigured() {
        if (_configured) {
            throw new JDOUnsupportedOptionException(
                    I18NHelper.getMessage(
                            messages,
                            "connection.connectionfactoryimpl.configured")); //NOI18N
        }
    }
    ...
    private void assertConnectionWait() {
        ...
        } else if (msInterval < 0 || msInterval > msWait
                || (msWait > 0 && msInterval == 0)) {
            throw new JDOUserException(
                    I18NHelper.getMessage(
                            messages,
    ...
    /**
     * INTERNAL Attempts to create new connectionManager Throws
     * JDOFatalException
     */
    ...
    private void unsupported() {
        throw new JDOUnsupportedOptionException(
                I18NHelper.getMessage(
                        messages, "connection.connectionfactoryimpl.notsupported")); //NOI18N
    }
