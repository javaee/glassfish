 ...
 * simple {@link java.lang.StringBuffer} "println" type statements. <p> The
 * order of the generated code in this implementation depends on the
 ...
 * category).  Specifying the ordered parameter in the constructor will lead to
 * a slightly different order: generation of fields and methods interspersed
 * among each other in exactly the order they were added (no member
 * categories).
 * @author raccah
 */
public final class IOJavaClassWriter implements JavaClassWriter {
    ...
    /**
     * Creates a new instance of IOJavaClassWriter which maintains the order of
     * member input but groups them by category.
     ...
     * preserved within category groups.  If <code>false</code>, the generation
     * of fields and methods will be interspersed among each other in exactly
     * the order they were added with no member categories.
     */
    ...
    public void setClassDeclaration(final int modifiers,
        ...
        writerHelper.writeln(
                modifierString + ((modifierString.length() > 0) ? " " : "") + "class " + className);	// NOI18N
    ...
    public void addField(final String name, final int modifiers, String type,
        ...
        final FormattedWriter writerHelper = new FormattedWriter();
        final String fieldString = Modifier.toString(modifiers) + ' ' + type
                + ' '
                + name;

        writerHelper.writeComments(comments);
        writerHelper.writeln(
                fieldString + ((initialValue != null)
                ? (" = " + initialValue) : "") + ';');		// NOI18N
    ...
    public void addConstructor(final String name, final int modifiers,
            ...
            final String[] exceptions, final String[] body,
            final String[] comments) {
        addMethod(
                name, modifiers, null, parameterNames, parameterTypes,
    ...
    public void addMethod(final String name, final int modifiers,
        ...
        addMethod(
                name, modifiers, returnType, parameterNames, parameterTypes,
    ...
    private void addMethod(final String name, final int modifiers,
        ...
        final String signature = createMethodSignature(
                name, modifiers, returnType, parameterNames, parameterTypes,
                exceptions);
        final FormattedWriter writerHelper = new FormattedWriter();
        final int n = (body != null ? body.length : 0);
    ...
    static private String createMethodSignature(final String name,
            final int modifiers, String returnType,
            final String[] parameterNames, final String[] parameterTypes,
            final String[] exceptions) {
        int i, count = (parameterNames != null ? parameterNames.length : 0);
        final List parameterPairs = new ArrayList();
        final FormattedWriter writerHelper = new FormattedWriter();
        ...
        writerHelper.write(
                ((returnType != null) ? returnType + " " : "") + name);			// NOI18N
        ...
        for (i = 0; i < count; i++) {
            writeListElement(
                    i, count, parameterTypes[i] + ' ' + parameterNames[i], writerHelper);
        }
        writerHelper.write(")");						// NOI18N
