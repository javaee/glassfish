...
/**
 * This class provides a default implementation of the com.sun.persistence.utility.logging.Logger
 * interface which implements most methods and/or delegates them to a few
...
abstract public class AbstractLogger implements Logger {
    ...
    private static final ResourceBundle _messages = I18NHelper.getInstance(
            AbstractLogger.class).getResourceBundle();
    ...
    /**
     * Creates a new AbstractLogger.  The supplied class loader or the loader
     * which loaded this class must be able to load the bundle.
    ...
    public AbstractLogger(String loggerName, String bundleName,
        ...
        _bundle = 
                I18NHelper.getInstance(bundleName, loader).getResourceBundle();
    }
    ...
    /**
     * Return whether logging is enabled at the FINE level.  This method is not
     * exact because to make it accurately reflect the logging level we would
     * have to include the JDK 1.4 java.util.logging.Level class. This method
     * does not delegate to isLoggable(FINE) for performance reasons.
     ...
     * logger.  This check is based on the Logger's effective level, which may
        ...
        if (isLoggable(FINER)) {
            ...
            if ((params != null) && (params.length > 0)) {
                ...
                args = new String[]{sourceClass, sourceMethod,
                                    StringHelper.arrayToSeparatedList(
    ...
    /**
     * This method returns a string from the bundle file if possible, treating
     * the message argument as the key.  If no such key is found in the bundle,
    ...
    /**
     * This method returns a string with a formatted prefix which depends on the
     * level.
