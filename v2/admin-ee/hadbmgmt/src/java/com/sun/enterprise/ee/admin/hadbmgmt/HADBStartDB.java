...
public class HADBStartDB
{
    ...
    public Object[] startDB() throws HADBSetupException
    {
        ...
        String[] commands = info.getStartCommands();
        HADBMExecutor exec = new HADBMExecutor(info.getExecutable(), commands);
        int exitValue = exec.exec();
        if(exitValue != 0)
        {
            // already running: "hadbm:Error 22096"
            if(exec.isHadbmError(22096))
            {
                // this is NOT an error.  They asked to start the DB and it is already running.
                // 6535895 WBN March 20, 2007
                String s = StringHelper.get("hadbmgmt-res.AlreadyRunning", info.getClusterName());
            ...
            {
                // unknown error...
                throw new HADBSetupException("hadbmgmt-res.StartFailed",
                    new Object[] {info.getClusterName(), "" + exitValue, exec.getStdout(), exec.getStderr()} );
            }
        }
        else
        {
            String s = StringHelper.get("hadbmgmt-res.StartSucceeded", info.getClusterName());
            LoggerHelper.fine(s);
            info.addMsg(s);
        }
