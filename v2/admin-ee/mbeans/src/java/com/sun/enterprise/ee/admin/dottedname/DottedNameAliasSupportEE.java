...
public class DottedNameAliasSupportEE extends DottedNameAliasSupport{
    ...
    public static String resolveScope( final DottedNameClusterInfo clusterInfo,
        ...
        final boolean	scopeIsClusterName;
        try {
            scopeIsClusterName = clusterInfo.getClusterNames().contains( actualScopeName );
        } catch (Exception e) {
            throw new DottedNameServerInfo.UnavailableException(e);
        }

        if ( scopeIsClusterName ) {
            if ( isAliasedDomain( dn ) ) {
                ...
                try {
                    actualScopeName	= clusterInfo.getConfigNameForCluster( scopeNameIn );
                } catch (Exception e) {
                    throw new DottedNameServerInfo.UnavailableException(e);
                }
            }
            ...
            actualScopeName = resolveScope(serverInfo, dn);
        }
