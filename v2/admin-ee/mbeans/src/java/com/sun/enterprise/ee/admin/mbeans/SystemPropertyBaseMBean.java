...
{    
    private static final StringManager _strMgr = 
        StringManager.getManager(SystemPropertyBaseMBean.class);

    private static Logger _logger = null;    
    
    public SystemPropertyBaseMBean() {
        super();
    }
    ...
    {       
        String name = getName();
        try { 
            return getPropertyConfigBean().listSystemProperties(name,
                inherit);
        } catch (Exception ex) {        
            throw getExceptionHandler().handleInstanceException(ex, 
    ...
    {       
        String name = getName();
        try { 
            getPropertyConfigBean().createSystemProperties(props,
                name);
        } catch (Exception ex) {        
            throw getExceptionHandler().handleInstanceException(ex, 
                getLogMessageId(), name);
    ...
    {       
        String name = getName();
        try { 
            getPropertyConfigBean().deleteSystemProperty(propertyName,
                name);
        } catch (Exception ex) {        
            throw getExceptionHandler().handleInstanceException(ex, 
                getLogMessageId(), name);
