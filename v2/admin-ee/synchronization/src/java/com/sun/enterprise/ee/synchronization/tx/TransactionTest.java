...
class Worker implements Runnable {
    ...
    public void run() {
        try { 
            ...
            if (_tx.isCommited()) {
                System.out.println("TX Commited " + _id);
            }
        }
    }
...
public class TransactionTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            int txN = Integer.parseInt(args[0]);
            if (txN <= 0) return;

            Transaction txn = mgr.begin(txN, 0);

            Worker[] workers = new Worker[txN];
            Thread[] threads = new Thread[txN];
            for (int i=0; i<workers.length; i++) {
                workers[i] = new Worker(txn, i);
