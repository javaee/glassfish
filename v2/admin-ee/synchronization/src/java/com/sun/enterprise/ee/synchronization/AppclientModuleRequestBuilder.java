...
/**
 * Builds a synchronization request for an application client 
 * module.
 *
 * @author Nazrul Islam
 */
public class AppclientModuleRequestBuilder extends RequestBuilderBase {
    
    public AppclientModuleRequestBuilder(ConfigContext ctx, String serverName) {
        super(ctx,serverName);
    }

    String getApplicationName(ConfigBean cb) {
        return ((AppclientModule)cb).getName();
    }

    public ApplicationSynchRequest build(AppclientModule appclientModule) {
        ApplicationSynchRequest asr = super.build(appclientModule);

        SynchronizationRequest sr = buildEjbDir(appclientModule);
    ...
    SynchronizationRequest buildAppDir(ConfigBean cb) {
        //String src = ((AppclientModule)cb).getLocation();
