 ...
 * @author Nazrul Islam
 */
public class JmxGroupRequestMediator extends BaseGroupRequestMediator {
     ...
     */
    public JmxGroupRequestMediator(DASPropertyReader p, 
            SynchronizationRequest[] reqs, Transaction tx) {
            
        _tx = tx;
        _mediators = new JmxRequestMediator[reqs.length];

        for (int i=0; i<reqs.length; i++) {
            _mediators[i] = new JmxRequestMediator(p, reqs[i], tx);
        }
    }
}
