...
/**
 * Builds a synchronization request for an EJB module.
 *
 * @author Nazrul Islam
 */
public class EjbModuleRequestBuilder extends RequestBuilderBase {
    
    public EjbModuleRequestBuilder(ConfigContext ctx, String serverName) {
        super(ctx,serverName);
    }

    String getApplicationName(ConfigBean cb) {
        return ((EjbModule)cb).getName();
    }

    public ApplicationSynchRequest build(EjbModule ejbModule) {
        ApplicationSynchRequest asr = super.build(ejbModule);

        SynchronizationRequest sr = buildEjbDir(ejbModule);
    ...
    SynchronizationRequest buildAppDir(ConfigBean cb) {
        //String src = ((EjbModule)cb).getLocation();
