...
public class ElementNameHandler extends BaseHandler {
    /**
     *
     * Constructor.
     *
    ...
    public Object handleAttribute(ObjectName peer, String attribute,
        ...
        if (type == null) {
            Element elem = getCMM_MBeanDescriptor();
            type = elem.getAttribute("name");
            if (type != null) {
                if (type.startsWith("CMM_"))
        ...
        String name = (String) _handlerProperties.get("name");
        if (name == null) {
            String attribToUseForName = (String) _handlerProperties.get("name-attribute");
            if (attribToUseForName == null)
                attribToUseForName = "name";
            
            String[] names = attribToUseForName.split(",");
            if(names != null && names.length > 0) {
                name = "";
                for (int i = 0; i < names.length; i++) {
                    String name1 = peer.getKeyProperty(names[i]);
                    if (name1 != null) {
                        if (name.length() > 0)
                            name = name + ",";
                        name = name + name1;
