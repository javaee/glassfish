 ...
 */
@Singleton
public class BBeanTranslator implements XmlDomTranslationService {
    ...
    public XmlDomHubData translate(XmlDomHubData hk2ConfigBeanData) {
        if (hk2ConfigBeanData.getType().equals(HubIntegrationTest.BBEAN_TAG)) {
        ...
        if (hk2ConfigBeanData.getType().equals(HubIntegrationTest.CBEAN_TAG)) {
            ...
            String oldInstanceName = hk2ConfigBeanData.getInstanceKey();
            
            int index = oldInstanceName.lastIndexOf('.');
            if (index < 0) {
                newInstanceName = oldInstanceName;
            }
            else {
                newInstanceName = oldInstanceName.substring(index + 1);
            }
